...
Page({
  ...
  // 分享海报：成品图 + 品牌条（抖音版不带小程序码：wxacode 是微信能力，抖音码接口待后端接入后恢复）
  makePoster(url) {
    tt.showLoading({ title: '海报生成中' });
    // 小程序码暂缺，直接走无码版海报（qrPath=null 退化）
    const qrPromise = Promise.resolve(null);
    tt.downloadFile({
      url,
      success: (dl) => {
        tt.getImageInfo({
          ...
          fail: () => { tt.hideLoading(); tt.showToast({ title: '海报生成失败', icon: 'none' }); },
        });
      },
      fail: () => { tt.hideLoading(); tt.showToast({ title: '下载原图失败', icon: 'none' }); },
    });
  },
      ...
      tt.canvasToTempFilePath({
        ...
        success: (r) => {
          tt.saveImageToPhotosAlbum({
            ...
            success: () => tt.showToast({ title: '海报已存相册，去朋友圈晒吧' }),
  ...
  saveAll(urls) {
    ...
    const step = (i) => {
      if (i >= urls.length) {
        tt.hideLoading();
        tt.showToast({ title: '整组已存到相册' });
        return;
      }
      tt.downloadFile({
        url: urls[i],
        success: (r) => new Promise(res => tt.saveImageToPhotosAlbum({
          filePath: r.tempFilePath, success: res, fail: res,
        })).then(() => {
          tt.showLoading({ title: `保存中 ${i + 1}/${urls.length}` });
          step(i + 1);
        }),
        fail: () => { tt.hideLoading(); tt.showToast({ title: '保存失败，检查相册权限', icon: 'none' }); },
      });
    };
    step(0);
  },
