...
Page({
  ...
  onLoad() {
    ...
    const landingSeen = !!tt.getStorageSync('landing_seen');
    if (!landingSeen) {
      tt.navigateTo({ url: '/pages/landing/landing' });
    }
    // 老用户跳过欢迎语（首次用户才看引导三句话）
    const saved = tt.getStorageSync('mp_order');
  ...
  onShow() {
    ...
    if (!tt.getStorageSync('landing_seen')) return;  // 落地页还盖着
    this._ensured = true;
    this.ensureOrder();
  },
  ...
  ensureOrder() {
      ...
      app.globalData.tokenPromise.then(() => {
        app.req('/api/mp/join', 'POST', { share_token: share, open_token: app.globalData.openToken })
          .then(res => {
            ...
            tt.setStorageSync('mp_role', app.globalData.myRole);
            this.setData({ order: res.order });
            app.globalData.order = res.order;
            tt.setStorageSync('mp_order', res.order);
            if (res.role === 'B') {
    ...
    const saved = tt.getStorageSync('mp_order');
    if (saved && saved.order_no) {
      app.globalData.myRole = tt.getStorageSync('mp_role') || 'A';
      this.setData({ order: saved });
      app.globalData.order = saved;
      this.resume(saved);
      return;
    }
    // 等 tt.login 拿到 openid 后再建订单，否则 open_token 为空会 422
    app.globalData.tokenPromise.then(() => {
      ...
      app.req('/api/mp/orders', 'GET', { open_token: app.globalData.openToken })
        .then(res => {
          ...
          // 找回提示用对话内按钮（tt.showModal 在部分真机场景静默失败，会把流程卡死）
  ...
  openAuth(role) {
    ...
    app.req('/api/mp/auth-session', 'GET', { order_no: this.data.order.order_no, role })
      .then(res => {
        ...
        tt.setClipboardData({
          data: res.url,
          success: () => tt.showModal({
            title: '去认证',
            content: '链接已复制。请用手机浏览器（Safari/系统浏览器）打开，完成刷脸后回到这里点「我已完成认证」',
            confirmText: '我已完成',
            cancelText: '稍后',
  ...
  push(role, text, action, images) {
    ...
    setTimeout(() => {
      ...
      tt.pageScrollTo({ scrollTop: 99999, duration: 200 });
    }, 50);
  },
    ...
    app.req('/api/mp/auth-status', 'GET', { order_no: this.data.order.order_no })
      .then(res => {
        ...
        if (res.order.auth_ok) {
          ...
          tt.showToast({ title: '还没查到认证结果，完成后再点我哦', icon: 'none' });
        }
      })
      .catch(e => {
  ...
  goUpload() {
    tt.navigateTo({ url: this.data.dockPage });
  },
  ...
  // ---- 语音输入（抖音版已下线：WechatSI 是微信插件，入口按钮已注释，留函数备用）----
  openVoice() {
    tt.showToast({ title: '抖音版暂不支持语音输入', icon: 'none' });
  },
  ...
  startVoice() {
    ...
    tt.authorize({
      ...
      fail: () => tt.showToast({ title: '需要录音权限哦', icon: 'none' }),
    });
  },
  ...
  sendChat() {
    ...
    if (this.data.pendingImages.some(i => i.status === '上传中')) {
      tt.showToast({ title: '图片还在上传中', icon: 'none' });
      return;
    }
    if (!this.data.order) {
      tt.showToast({ title: '订单还没准备好，稍等一下', icon: 'none' });
      return;
    }
  ...
  runChatAction(action) {
    if (action.type === 'navigate' && action.page) {
      ...
      tt.setStorageSync('mp_order', order);
