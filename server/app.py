...
- {{"type": "storylab_trailer", "fields": {{"tone": "情绪基调", "usage": "用途与隐私", "voice": "声音处理", "must_include": "必含时刻", "avoid": "禁区"}}}} 「故事片场」短片偏好收集（功能自称「故事片场」，用户口语说"预告片/视频/片子"都算）：用户表达"把花絮做成片子/做一支预告片/剪成视频"时用（【素材理解】为空时先引导上传视频花絮）。**一次只问一个问题**（5 个高杠杆问题：①想要什么情绪基调 ②用途与隐私（自己留念/发朋友圈/婚礼现场播）③声音（保留现场原声/加背景音乐/要不要旁白）④必含的时刻 ⑤禁区（不想出现的画面）），用户答了什么就在 fields 带什么（没答的留空字符串），严禁一次问完全部；对话历史里已有的答案直接带上，不重复问；信息在对话里已经足够时直接补齐全部 fields。每轮用户答完后，若 5 项未集齐，必须接着问下一个没答的问题（对照对话确认哪些已答），不许只确认收到却不追问。5 项未齐时严禁输出任何"已记录"类完成话术；5 项集齐后也不要自己输出这类字样（系统会自动补确认与卡片并开剪），且严禁承诺出片时间
...
- "把花絮做成片子/做一支预告片/剪成视频/素材能做什么/做一个(搞笑/创意)视频/用照片做视频" → storylab_trailer（素材是照片也行：must_include 里记"使用聊天里的照片"；【素材理解】为空且用户没发图时先引导上传素材）；用户问素材里有什么 → storylab_summary。用户追问"什么时候能出片/现在就要"时诚实回答：5 项偏好集齐后系统自动开剪（「故事片场」队列），先陪 TA 把偏好问完
    ...
    try:
        ...
        if done5:
            ...
            if missing5:
                ...
                prefs5["_done"] = 1
                _mp_touch(conn, body.order_no, storylab_prefs=json.dumps(prefs5, ensure_ascii=False))
                # 「故事片场」真出片：幂等建 storylab_film 任务（与智能体路径同一函数）
                chat_agent.enqueue_storylab_film(conn, body.order_no, _MP_CHAT_AGENT_DEPS)
                reply = "齐活！你的「故事片场」开机准备完成🎬 我这就开剪，好了第一时间喊你来看！"
                if "旁白" in str(prefs5.get("voice", "")):
