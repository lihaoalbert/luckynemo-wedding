...
Page({
  ...
  goBack() { wx.navigateBack({ fail: () => wx.redirectTo({ url: '/pages/moka/moka' }) }); },
  ...
  toggleFav() {
    ...
    app.req('/api/mp/fav', 'POST', {
      ...
      if (fav) wx.showToast({ title: '已收藏，在「我的」里随时找到', icon: 'none' });
    }).catch(e => {
  ...
  toggleSelMode() {
    ...
    this.updateCta();
    if (selMode) wx.showToast({ title: '点右上角圆圈选片，可多张', icon: 'none' });
  },
