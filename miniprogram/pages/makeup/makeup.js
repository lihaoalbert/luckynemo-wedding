...
Page({
  ...
  confirm() {
    ...
    if (!m) {
      wx.showToast({ title: '先选一款妆容哦', icon: 'none' });
      return;
    }
    ...
    this.poller = setInterval(() => {
      app.req('/api/mp/order/' + app.globalData.order.order_no).then(res => {
        ...
        if (job && job.status === 'done' && job.result && job.result.url) {
          ...
          clearInterval(this.timer);
          this.setData({ phase: 'pick' });
          wx.showModal({ title: '生成失败', content: '这款妆容生成失败了，换一款试试？', showCancel: false });
        }
      }).catch(() => {});
    }, 4000);
  },
  ...
  next() {
    ...
    if (this.data.role === 'B' && app.globalData.myRole === 'B') {
      wx.showToast({ title: '你的部分完成啦 🎉', icon: 'none' });
      setTimeout(() => wx.switchTab({ url: '/pages/chat/chat' }), 800);
      return;
    }
    if (this.data.mode === 'couple' && this.data.role === 'A') {
      wx.redirectTo({ url: '/pages/makeup/makeup?role=B' });
      return;
    }
