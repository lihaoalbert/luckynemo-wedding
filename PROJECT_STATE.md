...
- **P4 端到端验收完成（2026-08-29）：`films/benben-xuchi/_storylab_auto/assemble.py`（纯 ffmpeg/PIL，0 生成调用，不动 toolkit）按 storyboard_v2.json 自动剪出 `trailer_auto.mp4`（43.3s / 720x1280 / 24fps / 片名卡+15 镜+E3 转场+2.5s AI 标识卡，GB 45438 合规）**。QC 13 处抽帧目检通过；执行修正 3 处（15 镜出点钳 0.01s、4/10 镜回切 4K 原片且发现 4K 为竖屏旋转存储、"上下遮幅"意图自动退化为直出）。与人工版对比在 `_storylab_auto/REPORT.md`：自评 6.5/10 vs 人工 7.5/10，差距在节拍卡点与调速（路径明确：onset 检测/setpts+minterpolate）。产品化清单 8 项（OSS 下载/节拍对齐/字幕 schema/调速执行/字体模板/worker storylab_trailer 任务/QC 自动化/4K 回切固化）
...
- **v4 已上线（2026-08-07）**：main 合并 f845f8e → 96822e1（含 fav 鉴权收紧）；ECS `/var/www/luckynemo/moka/` 180 模板 + index.json v4、`/opt/luckynemo/server/` app.py/mp_worker.py 已同步，两服务 active；线上冒烟 catalog=180模板/20系列(全9变体)/6分组/4节点、mk301.png 200、lv001.png 404、fav 伪造 token 401 ✓。发现并修复：仓库 templates/ 残留 56 张 lv 旧卡被 rsync 误回线上，已删线上+删库（7908671，lovers_draft 存档保留）。前端已随 v1.1.4 发布（2026-08-11）。**待办：占位稿 Seedream 重出替换**
...
- **me 页改版（2026-08-07，已随 v1.1.4 发布）**：生成的照片挪二级页 `pages/photos/`（网格 + 长按保存/删除，me 页只留入口行，防几十张图撑长页面）；全屏预览改整组 urls 传参，系统自带左右滑动（上传照片/生成照片都支持）
...
- 2026-08-29 **storylab 素材理解接入小程序 chat v1 实装完成（跨组文件，未提交未部署）**：worker 新增 `storylab_ingest` 任务类型（mp_worker.py `run_storylab_ingest`，复用 P4 `storylab_ingest.py` 的探测/抽帧/motion/音峰/VLM 打标函数，惰性 import toolkit；限额每单 20 段、单段 ≤60s 超限跳过记日志、单段 VLM 失败重试 1 次再标 error 不阻塞整单、已打标段自动跳过=天然幂等）；新表 `mp_storylab_tags`（order_no/oss_key/duration/tags_json/highlight_window/motion_level/highlight/status/error，db_compat MYSQL_SCHEMA + app.py _migrate 双路）+ `mp_orders.storylab_prefs` 列（MYSQL_ALTERS + SQLite ALTER 双路）；触发点=`/api/uploads/sign` 登记 video/* 且 who 是 mp 订单时幂等 queued（`_storylab_maybe_queue`，触发失败不影响上传）；chat：`_mp_storylab_summary` 聚合注入 M3 system prompt（【素材理解】段：总数/时刻分布/高光≥4/开场候选 top2/全量逐段 caption），新增动作 `storylab_summary`（素材摘要卡，卡片=「想做一支预告片吗」入口，chat.js onAction 代发意图进收集流）与 `storylab_trailer`（5 个高杠杆问题逐轮收集，服务端合并落 storylab_prefs，集齐后确认卡+"生成能力即将开放"，v1 不做真实生成）；`_mp_storylab_fact_check` 反编造兜底（回复称"没发现"但标签命中关键词时改判事实回答——abab6.5s-chat 对长 prompt 检索不稳，实测必踩）。**跨组文件（app.py/mp_worker.py/chat.js）改动已在群内同步用，合并 main 前先沟通**。本地 SQLite 全链路 E2E 通过（3 短段+1 超限段：幂等触发→worker 30s 打标 3/3 零失败→开场摘要卡/素材问答/收集流+_prefs 落库全过）；OSS 测试对象与本地测试库已清理。遗留：①abab6.5s-chat 对"一次只问一个"纪律遵守不稳（靠服务端 missing 兜底落库正确）②素材问答只防漏报，模型虚报"有"暂无兜底 ③确认卡图片暂借 mk005.png，storylab 无独立卡片图 ④合并 main 时 PROJECT_STATE.md 需人工对齐（本文件基于旧 main）
...
- 2026-08-29 **storylab P4 端到端验收：分镜→成片全自动化跑通**（产物在主仓 films/，未提交）：`films/benben-xuchi/_storylab_auto/assemble.py`（纯 ffmpeg/PIL，0 生成调用）输入 storyboard_v2.json 自动剪出 `trailer_auto.mp4`（43.3s / 720x1280 / 24fps CFR / 音轨 BGM 淡入淡出+原声混流 / 片名卡+15 镜+E3 转场+2.5s AI 标识卡合规在位）。执行修正 3 处：第 15 镜出点 43.1→43.09 钳制；第 4/10 镜回切 4K 原片（意外发现 4K raw 是竖屏旋转存储，"上下遮幅"意图自动退化为全画幅直出且效果更优）；第 12 镜按 0 生成纪律用 P1 现成 E3 背景虚化裁 2.5s 替代 AI 转场。QC：13 处抽帧目检（含 3 个切点边界帧）+ ffprobe 全参验证 + 17 段时长对账（偏差 ≤0.02s）全过。对比报告 `_storylab_auto/REPORT.md`：自评 6.5/10 vs 人工 7.5/10（差在节拍卡点/调速/黄金瞬间精度，路径明确）；产品化清单 8 项（OSS 下载、onset 节拍对齐、字幕进分镜 schema、setpts+minterpolate 调速、Noto 字体与模板卡、worker storylab_trailer 任务、QC 自动化、4K 回切固化）
...
- 2026-08-11 **iOS 退款善后闭环**：`xpay_refund_notify` 不再只记日志——`_handle_ios_refund` 匹配支付单（①wechat_order_id：mp_pay_orders 新列，Midas 推送 best-effort 回填；②回退"金币数+支付日"唯一匹配，含 refunded 状态保证重复推送幂等不误报）→ 标记 refunded + 回收未消费额度（Python 侧 max(0,x)，因 MySQL MAX() 是聚合与 SQLite 标量方言冲突），匹配不到只告警人工处理。本地三场景单测过；线上冒烟误伤 AXEZ 真实单（冒烟用的日期恰好唯一命中），已即刻回滚（status/paid_count 复原）——**教训：冒烟事件的日期/金币数要避开真实数据**。已部署两服务 active
- 2026-08-11 **数据库 SQLite → RDS MySQL 迁移完成**（内网 rm-uf6px83tcbt52z3xc.mysql.rds.aliyuncs.com / 库 lucky_nemo / MySQL 8.4.7）：新增 `server/db_compat.py` 双后端兼容层（`?`→`%s` 含字面 `%` 转义——pymysql 用 % 格式化参数，`LIKE 'image/%'` 未转义曾致 500；`datetime('now')`→`NOW()`；`ON CONFLICT...DO UPDATE`→`ON DUPLICATE KEY UPDATE`；`ON CONFLICT...DO NOTHING`/`INSERT OR IGNORE`→`INSERT IGNORE`；Row 下标+列名双访问；row_factory no-op），app.py `_db()` 与 mp_worker.py `db()` 按 `DB_BACKEND` 环境变量切换（默认 sqlite，本地开发不受影响）；`migrate_to_mysql.py` 一次性迁移 13 表 314 行（REPLACE INTO 幂等）；PyMySQL 2.2.8 已入 requirements + ECS venv；ECS .env 已配 MYSQL_* + DB_BACKEND=mysql。冒烟：order/catalog/me 读 + feedback 写落 MySQL ✓，两服务 active。**回滚：ECS .env 删 DB_BACKEND=mysql 重启即回 SQLite（本地 app.db 未动但自此冻结）**。另：微信消息推送用户已在 MP 后台配置完成（Token 先配好，GET 验签通过），iOS 退款问询链路闭环。迁后首修：`/api/mp/pay_orders` 的 `ORDER BY rowid` 是 SQLite 隐式列，MySQL 1054 → 改 `created_at DESC`（订单中心加载失败即此，已上线恢复）；全仓已扫无其它 SQLite 专属 SQL。待观察：连接每请求新建，量级大后再上连接池
