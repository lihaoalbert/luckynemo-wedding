...
- r2v 新范式落地（2026-08-06）：分镜加顶层 assets 声明（characters/scenes/props）+ 每镜 refs 标签；`assets` 子命令生成 3 场景（white_studio/dark_studio/night_street，四宫格竖版 1440x2560 已品控合规）+ 3 道具（bouquet/ring/veil）并入新素材组 group-20260806222635-z8mjw；manifest `film/refs_assets/refs_manifest.json`；人物形象卡（bride_gown/groom_suit，asset-20260806225621-242d7 / asset-20260806225706-84ktz）shot_05 服装锁定验证通过；构图参考定为剪影版（`layouts` 子命令首帧转剪影，20 镜已生成 `film/layouts/`）；每镜 refs.characters 精确化（单人镜单卡）。**用户定剪到 1分16秒：分镜砍为 17 镜 76s（去掉 18/19/20 夜街段），全量 17 镜全要素草稿重跑中（旧片段存档 clips_draft_v2_旧r2v范式/）**
